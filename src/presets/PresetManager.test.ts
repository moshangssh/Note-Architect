...
const createManager = (presets: FrontmatterPreset[] = []) => {
	...
	const save = jest.fn(async (incoming?: NoteArchitectSettings) => {
		if (incoming) {
			settings.frontmatterPresets = incoming.frontmatterPresets.map((preset) => ({
				...preset,
				fields: preset.fields.map((field) => ({ ...field })),
			}));
			settings.recentlyUsedTemplates = [...incoming.recentlyUsedTemplates];
			settings.templateFolderPath = incoming.templateFolderPath;
		}
		return settings;
	});
	const settingsManager = {
		...
		save,
		addPreset: jest.fn(async (preset: FrontmatterPreset) => {
		...
		deletePreset: jest.fn(async (presetId: string) => {
			...
			settings.frontmatterPresets.splice(index, 1);
			await save(settings);
		}),
		updatePresetFields: jest.fn(async (presetId: string, fields: FrontmatterField[]) => {
			const preset = settings.frontmatterPresets.find((item) => item.id === presetId);
			if (!preset) {
				throw new Error(`未找到 ID 为 "${presetId}" 的预设`);
			}
...
describe('PresetManager 预设ID生成', () => {
	...
	it('应当在名称包含非ASCII字符时生成可读编码', () => {
		...
		expect(manager.generateUniquePresetId('示例配置')).toBe('preset-fmst8y');
	});
	...
	it('createPreset 在仅提供名称时应自动生成唯一ID并持久化', async () => {
		...
		expect(preset.id).toBe('preset-fmst8y');
