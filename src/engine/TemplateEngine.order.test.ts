...
describe('mergeFrontmatterWithUserInput frontmatter 顺序控制', () => {
	...
	it('应按照预设字段顺序输出 frontmatter', async () => {
		...
		const result = await mergeFrontmatterWithUserInput(app, {} as any, preset, templateFrontmatter, userFrontmatter);

		expect(Object.keys(result)).toEqual(['tags', 'status', 'create-data', 'modifieddata']);
	});

	it('应保留预设外字段并将其附加在末尾', async () => {
		const app = createMockApp();
