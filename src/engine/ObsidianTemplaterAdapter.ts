...
export class ObsidianTemplaterAdapter implements TemplaterPort {
  ...
  async processTemplate(template: Template): Promise<string> {
    ...
    if (
      !plugin?.templater ||
      typeof plugin.templater.parse_template !== "function"
    ) {
  ...
  private getPlugin(): TemplaterPlugin | null {
    ...
    const plugin =
      pluginManager.getPlugin?.("templater-obsidian") ??
      pluginManager.plugins?.["templater-obsidian"];
    return plugin && typeof plugin === "object" && "templater" in plugin
      ? (plugin as TemplaterPlugin)
      : null;
  }
}
