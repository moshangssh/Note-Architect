...
import type { Template, FrontmatterPreset, TemplateInsertionResult } from '@types';
...
export async function executeTemplateInsertion(
	...
	try {
		...
		try {
			...
			const frontmatterYaml = stringifyFrontmatterAsYaml(preparation.mergedFrontmatter);
 ...
 * @param template 模板对象
 */
function handleInsertionSuccess(result: TemplateInsertionResult, template: Template): void {
 ...
 * @param template 模板对象
 */
function handleInsertionFallback(result: TemplateInsertionResult, template: Template): void {
	if (result.templaterError) {
		notifyWarning(`${result.templaterError}，已使用原始模板内容`);
	}

	if (result.fallbackToBodyOnly) {
		notifyWarning('Frontmatter 更新失败，已插入模板正文。您的 frontmatter 数据已复制到剪贴板，可以手动在文件顶部添加。');
		return;
	}
}
 ...
 * @returns YAML 格式的字符串
 */
function stringifyFrontmatterAsYaml(frontmatter: Record<string, unknown>): string {
