...
---`;
    const { registrar, vault, plugin } = createRegistrar(initialContent);
    ...
    await (registrar as any).bindPresetToTemplate(file, presets[1], template);
    ...
    expect(template.content).toBe(updatedContent);
...
note-architect-config:
  - preset-a
---`;
    const { registrar, vault } = createRegistrar(initialContent);
    ...
    await (registrar as any).bindPresetToTemplate(file, presets[0], template);
