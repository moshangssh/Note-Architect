...
import { normalizeConfigIds, collectMatchingPresets, resolvePresetConfigIds } from '@utils/note-architect-config';
...
describe('resolvePresetConfigIds', () => {
  it('優先解析新版 note-architect-config 欄位', () => {
    const { ids, sourceKey } = resolvePresetConfigIds({
      'note-architect-config': ['preset-a', 'preset-b'],
      'fast-templater-config': ['legacy-a'],
    });
    expect(ids).toEqual(['preset-a', 'preset-b']);
    expect(sourceKey).toBe('note-architect-config');
  });

  it('向後相容 fast-templater-config 欄位', () => {
    const { ids, sourceKey } = resolvePresetConfigIds({
      'fast-templater-config': 'legacy-a',
    });
    expect(ids).toEqual(['legacy-a']);
    expect(sourceKey).toBe('fast-templater-config');
  });

  it('當沒有配置時返回空結果', () => {
    const { ids, sourceKey } = resolvePresetConfigIds(undefined);
    expect(ids).toEqual([]);
