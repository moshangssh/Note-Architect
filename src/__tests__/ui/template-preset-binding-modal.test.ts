...
jest.mock("@utils/async-ui", () => ({
  runWithBusy: jest.fn(
    async (_element: unknown, task: () => Promise<unknown>) => {
      await task();
      return undefined;
    }
  ),
...
describe("TemplatePresetBindingModal 多重綁定支援", () => {
  ...
  it("建構函數會初始化 existingIds 並保存為集合", () => {
    const modal = new TemplatePresetBindingModal(app, {
      ...
      onBind: async (_preset, _template) => {},
      onClear: async () => {},
    });

    const boundIds: Set<string> = (
      modal as unknown as { boundIds: Set<string> }
    ).boundIds;
    const optionIds: string[] | undefined = (
      modal as unknown as { options: { existingIds?: string[] } }
    ).options.existingIds;
  ...
  it("isPresetBound 會正確回傳綁定狀態", () => {
    ...
    const modalInternal = modal as unknown as {
      isPresetBound(id: string): boolean;
    };
  ...
  it("handleBind 成功後會更新狀態且不關閉視窗", async () => {
    ...
    await (
      modal as unknown as {
        handleBind: (preset: FrontmatterPreset, button: any) => Promise<void>;
      }
    ).handleBind(createPresets()[0], buttonStub);

    expect(onBind).toHaveBeenCalledWith(
      expect.objectContaining({ id: "preset-a" }),
      expect.objectContaining({ id: "template-001" })
    );
    expect(mockRunWithBusy).toHaveBeenCalledWith(
      buttonStub.buttonEl,
      expect.any(Function),
    ...
    expect(modalInternal.renderPresetList).toHaveBeenCalledWith({
      preserveScroll: true,
    });
  });
  ...
  it("handleClear 成功後會清空綁定並重新整理列表", async () => {
    ...
    const buttonStub = { buttonEl: {} } as any;
    await (
      modal as unknown as { handleClear: (button: any) => Promise<void> }
    ).handleClear(buttonStub);
