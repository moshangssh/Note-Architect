...
export class PresetManagerComponent {
  ...
  private renderHeaderAndActions(): void {
    ...
    // 描述
    const descEl = this.containerEl.createDiv({
    ...
    // 操作按钮区域
    const actionsContainer = this.containerEl.createDiv(
  ...
  private renderPresetListContainer(): void {
    const presetsListContainer = this.containerEl.createDiv(
   ...
   * 刷新预设列表
   */
  private refreshPresetsList(
    containerEl: HTMLElement,
    nextPresets?: FrontmatterPreset[]
  ): void {
  ...
  private async handleImportContent(raw: string): Promise<void> {
    ...
    try {
      ...
      const result = await this.presetManager.importPresets(content, {
        strategy,
      });
  ...
  private notifyImportResult(result: ImportPresetsResult): void {
    ...
    const renameSuffix =
      renameCount > 0 ? `，其中 ${renameCount} 个预设已自动重命名` : "";

    if (result.strategy === "replace") {
      notifySuccess(
        `已替换全部预设，共导入 ${importedCount} 个预设${renameSuffix}`
      );
    } else {
  ...
  private async confirmReplaceAll(): Promise<boolean> {
    const confirmModal = new SimpleConfirmModal(this.app, {
      title: "确认替换全部预设",
      message:
        "此操作将删除当前所有预设，并以导入文件中的配置完全替换，且无法撤销。确定继续吗？",
      confirmText: "确认替换",
      cancelText: "取消",
  ...
  private async handlePresetRename(
    ...
    if (!trimmedName) {
      context.nameInputComponent.setValue(originalName);
    ...
    const targetId = shouldUpdateId
      ? (modalResult.newId ?? "").trim()
      : originalId;
    ...
    context.nameInputComponent.setDisabled(true);
    ...
    try {
      ...
      await withUiNotice(
        async () => {
          let migrationResult: Awaited<
            ReturnType<typeof this.migratePresetBindings>
          > | null = null;
          ...
          try {
            const renameOutcome =
              await this.presetManager.renamePresetWithIdChange(
                originalId,
                trimmedName,
                {
      ...
      context.nameInputComponent.setDisabled(false);
    }
  }
      ...
      for (const file of templateFiles) {
        ...
        const result = updateFrontmatter(
          ...
          (frontmatter) => {
            ...
            const nextValue =
              updatedIds.length === 1 ? updatedIds[0] : updatedIds;
            return {
