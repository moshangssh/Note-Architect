...
export class NoteArchitectSettingTab extends PluginSettingTab {
	...
	private renderFrontmatterPresetsManager(containerEl: HTMLElement): void {
		...
		const refreshPresetsList = (nextPresets?: FrontmatterPreset[]) =>
			this.renderPresetsList(presetsListContainer, nextPresets);
		refreshPresetsList();
	...
	private renderPresetsList(
		...
		const refreshPresetsList = (nextPresets?: FrontmatterPreset[]) =>
			this.renderPresetsList(containerEl, nextPresets);

		renderPresetListUI({
			containerEl,
			presets: presets ?? this.presetManager.getPresets(),
			callbacks: {
	...
	private async handlePresetRename(
		...
		newName: string,
		onPresetsChanged: (presets?: FrontmatterPreset[]) => void,
		...
		try {
			await withUiNotice(
				...
				},
				{
					...
					fail: '重命名预设失败',
					onSuccess: () => {
						const latestPresets = this.presetManager.getPresets();
						onPresetsChanged(latestPresets);
					},
					onFail: () => {
						const latestPresets = this.presetManager.getPresets();
						onPresetsChanged(latestPresets);
