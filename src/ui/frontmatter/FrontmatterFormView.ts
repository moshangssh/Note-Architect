...
export class FrontmatterFormView {
	...
	private renderField(field: FrontmatterField): void {
		...
		if (!(field.key in initialData)) {
			if (field.type === 'multi-select') {
				...
				initialData[field.key] = typeof resolvedDefault === 'string' ? resolvedDefault : '';
			}
		}
	...
	updateField(key: string, value: unknown): void {
		const inputEl = this.fieldInputRefs.get(key);
		const multiSelectContainer = this.multiSelectFieldRefs.get(key);

		if (inputEl) {
			// 更新普通输入控件
			if ('value' in inputEl) {
				(inputEl as HTMLInputElement | HTMLSelectElement).value = this.coerceToString(value);
			}
		} else if (multiSelectContainer) {
			// 更新多选框
			const checkboxes = multiSelectContainer.querySelectorAll('input[type="checkbox"]') as NodeListOf<HTMLInputElement>;
	...
	markAllFieldsTouched(): void {
		this.touchedFieldKeys = new Set(this.options.preset.fields.map(field => field.key));
	}
