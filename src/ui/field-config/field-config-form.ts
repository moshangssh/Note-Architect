...
export class FieldConfigForm {
  ...
  private readonly multiSelectDefaultContainers = new WeakMap<
    FrontmatterField,
    HTMLElement
  >();
  ...
  private fieldRowRefs: Partial<
    Record<FieldValidationErrorKey, HTMLDivElement>
  > = {};
  private errorElements: Partial<
    Record<FieldValidationErrorKey, HTMLDivElement>
  > = {};
  ...
  private resetElementRefs(): void {
    this.keyInputComponent = undefined;
    this.labelInputComponent = undefined;
    this.typeSelectComponent = undefined;
    this.defaultInputComponent = undefined;
    this.descriptionInputComponent = undefined;
  ...
  private renderLabelInput(container: HTMLElement): void {
    ...
    const component = new TextComponent(inputContainer);
    component
      .setValue(this.config.field.label)
      .setPlaceholder("例如: 状态, 分类, 优先级");
    component.inputEl.addClass("note-architect-input-base");
    component.inputEl.addClass("note-architect-field-input");

    this.labelInputComponent = component;

    component.onChange((value) => {
      this.config.field.label = value.trim();
      this.notifyFieldChange();
    });
  ...
  private renderTypeSelect(container: HTMLElement): void {
    ...
    // 使用 DropdownComponent 替代手动创建的选择框
    ...
    component.onChange((value) => {
      this.handleTypeChange(value as FrontmatterFieldType);
    });
  }
  ...
  private renderDefaultInput(container: HTMLElement): void {
    const row = this.createFieldRow(container, {
      stacked:
        this.config.field.type === "multi-select" ||
    ...
    const contentContainer = row.createDiv(
      "note-architect-field-default-content"
    );
    ...
    const normalizedDefault =
      typeof this.config.field.default === "string"
    ...
    // 使用 TextComponent 替代手动创建的输入框
    const component = new TextComponent(contentContainer);
    component
      .setValue(normalizedDefault)
      .setPlaceholder("默认值或 Templater 宏（可选）");
    component.inputEl.addClass("note-architect-input-base");
    component.inputEl.addClass("note-architect-field-input");

    this.defaultInputComponent = component;

    if (!this.config.field.useTemplaterTimestamp) {
      this.manualDefaultCache.set(this.config.field, normalizedDefault);
    }

    component.setDisabled(this.config.field.useTemplaterTimestamp === true);
    component.onChange((value) => {
      this.config.field.default = value;
      if (!this.config.field.useTemplaterTimestamp) {
        this.manualDefaultCache.set(this.config.field, value);
      }
      this.notifyFieldChange();
    });
    ...
    if (this.config.field.type === "date") {
      ...
      this.renderDateAutoFillControls(container, component.inputEl);
    }
  }
  ...
  private renderOptionsSection(container: HTMLElement): void {
    ...
    new ButtonComponent(actions)
      .setButtonText("添加选项")
      .setClass("note-architect-field-options__add")
      .onClick(() => this.addOption(optionsListContainer));
  ...
  private renderDescriptionInput(container: HTMLElement): void {
    ...
    // 使用 TextAreaComponent 替代手动创建的文本域
    ...
    component.onChange((value) => {
      this.config.field.description = value.trim();
      this.notifyFieldChange();
    });
  }
  ...
  private updateFormValues(): void {
    ...
    if (this.descriptionInputComponent) {
      this.descriptionInputComponent.setValue(
        this.config.field.description ?? ""
      );
    }
  ...
  private updateDefaultValueControls(): void {
    ...
    if (!this.defaultInputComponent) {
      return;
    }

    const normalizedDefault = this.getScalarDefaultValue(this.config.field);
    this.defaultInputComponent.setDisabled(
      this.config.field.useTemplaterTimestamp === true
    );
    this.defaultInputComponent.setValue(normalizedDefault);
  ...
  private updateDateAutofillPreview(currentValue: string): void {
    if (
      !this.dateAutoFillCheckbox ||
      !this.dateAutoFillPreviewEl ||
      !this.defaultInputComponent
    ) {
      return;
    }
    const templaterEnabled = this.config.field.useTemplaterTimestamp === true;
    this.dateAutoFillCheckbox.setValue(templaterEnabled);
    this.defaultInputComponent.setDisabled(templaterEnabled);

    if (templaterEnabled) {
      const displayValue = currentValue || this.buildTemplaterDateExpression();
      if (this.defaultInputComponent.getValue() !== displayValue) {
        this.defaultInputComponent.setValue(displayValue);
      }
  ...
  private renderDateAutoFillControls(
    ...
    const previewEl = row.createDiv(
      "setting-item-description note-architect-date-autofill__preview"
    );
    this.dateAutoFillPreviewEl = previewEl;

    const applyAutoFillState = (
      enabled: boolean,
      ...
      if (enabled) {
        ...
        const manualValue =
          this.manualDefaultCache.get(this.config.field) ?? "";
    ...
    applyAutoFillState(this.config.field.useTemplaterTimestamp === true, {
      initial: true,
    });
    ...
    if (this.config.field.type === "multi-select") {
      ...
      const normalizedDefault =
        typeof currentDefault === "string"
          ? currentDefault
          : Array.isArray(currentDefault) && currentDefault.length > 0
          ? currentDefault[0]
          : "";
      ...
      if (
        ...
        normalizedDefault &&
        !allowedOptions.has(normalizedDefault)
      ) {
        this.config.field.default = "";
      } else {
        this.config.field.default = normalizedDefault;
   ...
   * 渲染多选复选框
   */
  private renderMultiSelectCheckboxes(
    container: HTMLElement,
    options: string[]
  ): void {
    const syncDefault = () => {
    ...
    options.forEach((option) => {
      ...
      const currentDefaults = Array.isArray(this.config.field.default)
        ? this.config.field.default
        : [];

      const checkbox = createMultiSelectCheckbox(
        container,
        option,
        currentDefaults.includes(normalizedOption),
        syncDefault,
        {
          labelClass: "note-architect-form-label",
          linkLabelToCheckbox: true,
        }
      );
   ...
   * 渲染单选单选按钮（包含“无默认值”选项）
   */
  private renderSingleSelectRadios(
    container: HTMLElement,
    options: string[]
  ): void {
    const radioGroupName = `select-default-${
      this.config.fieldIndex
    ...
    // 添加"无默认值"选项
    const noDefaultRow = container.createDiv(
    ...
    options.forEach((option) => {
      ...
      const optionRow = container.createDiv(
        "note-architect-checkbox-container"
      );
  ...
  private handleTypeChange(newType: FrontmatterFieldType): void {
    ...
    if (newType === "multi-select") {
      const normalizedOptions = Array.isArray(updatedField.options)
        ? new Set(
            updatedField.options.map((option) => option.trim()).filter(Boolean)
          )
        : undefined;
      updatedField.default = normalizeStringArray(
        updatedField.default,
        normalizedOptions && normalizedOptions.size > 0
          ? normalizedOptions
          : undefined
      );
    } else if (Array.isArray(updatedField.default)) {
  ...
  private buildTemplaterDateExpression(): string {
    const formatRaw =
      this.config.settingsManager?.getSettings().defaultDateFormat ??
  ...
  private toggleInputErrorClass(
    ...
    switch (kind) {
      ...
      case "label":
        ...
        break;
      case "options":
        this.optionsListContainer?.classList.toggle(
  ...
  private registerValidationSlot(
    kind: FieldValidationErrorKey,
    row: HTMLDivElement
  ): void {
