...
export class DetailPanelView {
  ...
  private readonly sectionAnimationHandlers = new Map<
    HTMLElement,
    () => void
  >();
  ...
  render(field: FrontmatterField | null, fieldIndex: number | null): void {
    ...
    this.subtitleEl.setText(
      field.key?.trim() ? `Frontmatter 键名：${field.key}` : "尚未设置键名"
    );
  ...
  focusOnFirstInput(): void {
    const firstInput = this.formContainerEl.querySelector<HTMLElement>(
  ...
  private applyValidationStateToForm(): void {
    ...
    if (this.activeFieldIndex === this.pendingValidation.fieldIndex) {
      this.fieldConfigForm.setValidationErrors(
        this.pendingValidation.errors ?? null
      );
    } else {
      this.fieldConfigForm.setValidationErrors(null);
    }
  ...
  private initializeLayout(): void {
    ...
    const titleGroup = this.headerEl.createDiv(
    ...
    this.actionsEl = this.headerEl.createDiv(
    ...
    new ButtonComponent(this.actionsEl)
      .setButtonText("← 返回列表")
      .setClass("note-architect-detail-panel__back-btn")
      .onClick(() => {
        this.config.onNavigateBack?.();
      });

    // 删除按钮
    const deleteBtn = new ButtonComponent(this.actionsEl)
      .setIcon("trash-2")
      .setClass("note-architect-detail-panel__delete-btn")
      .setTooltip("删除字段")
      .onClick(() => {
    ...
    this.deleteButtonEl = deleteBtn.buttonEl;
    ...
    this.formContainerEl = this.bodyEl.createDiv(
