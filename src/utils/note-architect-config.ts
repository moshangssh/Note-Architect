...
export const LEGACY_PRESET_CONFIG_KEYS = ['fast-templater-config'] as const;

export interface ResolvedPresetConfigIds {
  ids: string[];
  sourceKey: string | null;
}

/**
 * 從 frontmatter 解析預設配置欄位，支援舊版 fast-templater-config
 */
export function resolvePresetConfigIds(
  frontmatter: Record<string, unknown> | null | undefined,
): ResolvedPresetConfigIds {
  if (!frontmatter) {
    return { ids: [], sourceKey: null };
  }

  if (Object.prototype.hasOwnProperty.call(frontmatter, PRESET_CONFIG_KEY)) {
    return {
      ids: normalizeConfigIds(frontmatter[PRESET_CONFIG_KEY]),
      sourceKey: PRESET_CONFIG_KEY,
    };
  }
  ...
  for (const legacyKey of LEGACY_PRESET_CONFIG_KEYS) {
    if (Object.prototype.hasOwnProperty.call(frontmatter, legacyKey)) {
      return {
        ids: normalizeConfigIds(frontmatter[legacyKey]),
        sourceKey: legacyKey,
      };
    }
  }
...
/**
 * 移除所有與預設綁定相關的 frontmatter 欄位，確保寫回時不殘留舊鍵
 */
export function stripPresetConfigKeys(frontmatter: Record<string, unknown>): Record<string, unknown> {
  const next = { ...frontmatter };
  delete next[PRESET_CONFIG_KEY];
  for (const legacyKey of LEGACY_PRESET_CONFIG_KEYS) {
